...
export default class Checkout extends React.Component {
  // use a class state to declare state
  // instead of using a constructor and super
  state = {
  ...
  // Derived state
  // if it's an empty object, the form is invalid
  // Should only submit form if it's valid
  isValid() {
  ...
  handleChange = (e) => {
    e.persist(); // persist the event - only import for when using set state
    this.setState((state) => {
      // set the address to a copy of the current address
      // use the input's id to determine which property to set (using computed property syntax)
      return {
        address: {
          ...state.address,
  ...
  handleBlur = (event) => {
    event.persist();
    // any time a field is blurred, call setTouched
    this.setState((state) => {
